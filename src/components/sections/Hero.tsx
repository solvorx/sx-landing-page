 ...
 *   5. header
 * La geometría vive en la clase `.hero` de globals.css y es toda relativa al
 * alto de la sección, por eso el `min-h`: en pantallas muy bajas (móvil en
 * horizontal) el bloque de texto se saldría del polígono.
 */
export function Hero() {
  return (
    <section className="hero relative h-svh min-h-[620px] overflow-hidden bg-white">
      {/* Isotipo SX */}
      ...
      */}
      <div className="absolute inset-x-6 top-[30%] z-40 md:inset-x-auto md:top-(--text-top) md:right-[6%] md:left-(--text-left)">
        <h1 className="font-display text-[clamp(1.5rem,1vw+2.6vh,2.5rem)] leading-[1.15] font-semibold text-balance text-white">
          {hero.title}
        </h1>
        <p className="mt-5 text-[clamp(0.8125rem,0.4vw+1.1vh,1rem)] leading-relaxed text-pretty text-white/70">
          {hero.body}
        </p>
        <Button href={hero.cta.href} variant="onDark" className="mt-8">
          {hero.cta.label}
        </Button>
      </div>
